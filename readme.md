...
- Dans le fichier /client/Dockerfile.cli, vérifier que les 2 lignes suivantes ne sont **pas** commentées: `ARG VITE_API_BASE_URL` et `ENV VITE_API_BASE_URL=$VITE_API_BASE_URL`
...
- Créer les différentes variables et secrets Github dont la githubaction a besoin (Si l'hébergeur n'est pas OVH, les variables et secrets nommés "..._OVH" peuvent bien sûr être renommés sous Github et dans la githubaction.):
